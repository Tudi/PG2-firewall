...
static BOOL Main_OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam) {
	...
	//!!WOWBEEZ CODE - START!!//
FILE *tf=fopen("Port_scanners.log","w");
if(tf)
	fclose(tf);
tf=fopen("Connection_flooder.log","w");
if(tf)
	fclose(tf);
FILE *tfpdbg=fopen("debug.txt","w"); 

	//read our config file and set the port list
	FILE *conffile;
	conffile = fopen("beez_config.txt","r");
	if(conffile)
	{
		do
		{
			char readbuf[200];
			char *res;
			res = fgets(readbuf,200,conffile);
			if(!res)
				break; //end of file
			if(strstr(readbuf,"ADDPORT=")==readbuf)
			{
				unsigned short tempport;
				tempport = (unsigned short)atoi(readbuf+strlen("ADDPORT="));
				g_filter->SetAllowPort( tempport, 1);
//fprintf(tfpdbg,"found new port to open %u\n",tempport);
			}
			else if(strstr(readbuf,"SET_CONNECTION_COUNT_FOR_FLOOD=")==readbuf)
			{
				ULONG temp;
				temp = atoi(readbuf+strlen("SET_CONNECTION_COUNT_FOR_FLOOD="));
				g_filter->SetConsConLimit( temp );
			}
			else if(strstr(readbuf,"ADDALLOWEDIP=")==readbuf)
			{
				ULONG temp;
				temp = _byteswap_ulong(inet_addr(readbuf+strlen("ADDALLOWEDIP=")));
//fprintf(tfpdbg,"debug IP convert %u\n",temp);
				g_filter->AddPermaAllowIP( temp );
			}
			else if(strstr(readbuf,"DISABLEPORTSCANNERDETECTOR=")==readbuf)
			{
				int temp;
				temp = atoi(readbuf+strlen("DISABLEPORTSCANNERDETECTOR="));
				g_filter->DisablePortScanDetector( temp );
			}
			else if(strstr(readbuf,"DISABLECONNECTIONFLOODDETECTOR=")==readbuf)
			{
				int temp;
				temp = atoi(readbuf+strlen("DISABLECONNECTIONFLOODDETECTOR="));
				g_filter->DisableConnectionFloodDetector( temp );
			}
			else if(strstr(readbuf,"CLEARLOGFILESINTERVAL=")==readbuf)
			{
				unsigned int temp;
				temp = atoi(readbuf+strlen("CLEARLOGFILESINTERVAL="))*1000;
				g_filter->SetLogFlushInterval( temp );
			}
		}
		while (1);
		fclose(conffile);
	...
	//assign list and set values for the driver
	g_filter->RegisterConLimit();
//	g_filter->RegisterNewBanBuff();

fclose(tfpdbg);
	//!!WOWBEEZ CODE - END!!//

	//we need to make our setup before we start the hook itself
